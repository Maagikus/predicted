(() => {
    ...
    (() => {
        ...
        da.init();
        let script_parent = document.querySelector(".menu__list");
        let menuItem = script_parent.querySelectorAll(".menu__item");
        script_parent.addEventListener("click", (event => {
            let target = event.target.parentElement;
            console.log(target);
            if (target.classList.contains("menu__item")) {
                for (let i = 0; i < menuItem.length; i++) menuItem[i].classList.remove("active");
        ...
        const videoArray = document.querySelector(".main__image");
        const isSafari = () => {
            const userAgent = navigator.userAgent.toLowerCase();
            const Safari = /safari/.test(userAgent);
            /chrome/.test(userAgent);
            if (Safari) videoArray.insertAdjacentHTML("beforebegin", `<img src="@img/section-1/brain.png" alt="brain-image">`); else videoArray.insertAdjacentHTML("beforebegin", ` <video muted   autoplay="autoplay" data-src="./files/PREDICTUM.webm" loop="loop" >\n\t\t</video> `);
        };
        window.addEventListener("load", isSafari);
        window["FLS"] = true;
        isWebp();
        addTouchClass();
        menuInit();
